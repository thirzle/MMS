...
public class EditModule extends SessionCheck {
		...
		if(isLoggedIn(request, response)) {
			...
			if (request.getParameter("editButton") != null) {
				if (request.getParameter("editButton").equals(
					...
					System.out.println("(EditModule.java): Unbest�tigtes Modul wird bearbeitet");
				}
			}
	...
	private void loadFieldsA() {
		...
		for (TextualEntry entry : textualEntrys) {
			if (entry.getTitle().equals("K�rzel")) {
				fieldsTypeA.add(new String[] { "K�rzel", entry.getContent() });
				...
				entry.setPredefined(true);
			} else if (entry.getTitle().equals("Pr�fungsform")) {
	...
	private void handleButtonEvents(HttpServletRequest request,
		...
		if (request.getParameter("editModule") != null) {
			if (request.getParameter("editModule").equals("addRow")) {
				...
				System.out
						.println("(EditModule.java): Reihe zum Modul hinzugef�gt");
			} else if (request.getParameter("editModule").equals("saveModule")) {
				System.out
						.println("(EditModule.java): Modul f�r Sitzung gespeichert");
				response.sendRedirect("/SopraMMS/guiElements/home.jsp"+editUnapprovedModuleUrl);
				...
				EffortEntry effort = new EffortEntry("Pr�senzzeit", order++, pt);
				...
				if (!editUnapprovedModule) {
					...
					System.out.println("+++++++++");
					System.out.println("(EditModule.java): Bearbeitetes Modul --> �berschreibt Modul");
					System.out.println(editUnapprovedModule+"  url: "+editUnapprovedModuleUrl);
					System.out.println("+++++++++");
			...
			System.out.println("(EditModule.java): Reihe gel�scht");
			response.sendRedirect("/SopraMMS/guiElements/home.jsp"+editUnapprovedModuleUrl);
