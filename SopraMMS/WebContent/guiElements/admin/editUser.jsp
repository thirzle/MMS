...
<link rel="stylesheet" type="text/css"
	href="/SopraMMS/css/multiple-select.css">
...
<div>
	<table>
		...
		<tr>
			<td>Vorname:</td>
			<td><input oninput="generateLoginname()" class="inputField"
				form="newUserForm" type='text' id="firstnameCellText"
				name="firstnameCellText" value='<%=user.getFirstName()%>' /></td>
		</tr>
		<tr>
			<td>Nachname:</td>
			<td><input oninput="generateLoginname()" class="inputField"
				form="newUserForm" type='text' id="lastnameCellText"
				name="lastnameCellText" value='<%=user.getLastName()%>' /></td>
		</tr>
		<tr>
			<td>Email:</td>
		...
		<tr>
			<td>Rechte:</td>
			<td><select onchange="alert('change')" class="inputField"
				id="rightsSelect" style="width: 270px">
		...
		<tr>
			<td>Institut:</td>
			<td><select class="inputField" id="instituteSelect"
				style="width: 270px">
					<%
		...
		<%
			UserAdministration uAdmin = new UserAdministration();
				String curUrl = uAdmin.getCurriculum(user.getLogin());
				if(curUrl!=null){if(!curUrl.equals("")){
		%>
		<tr>
			<td>Lebenslauf:</td>
			<td><a href=<%=curUrl%> target="_blank"><%=curUrl%></a></td>
		</tr>
		<%
			} }
		%>
			...
			<td>
				<form id="newUserForm" action="/SopraMMS/ChangeUser" method="get">
					<input type="submit" name="Submit" id="saveButton"
						style="float: right;" value="Ändern" />
					<textarea name="rightsSelect" style="display: none;"
						id="rightsLabel"></textarea>
					<textarea name="instituteSelect" style="display: none;"
						id="instituteLabel"></textarea>
				</form>
			</td>
		</tr>
	</table>
...
<script type="text/javascript"
	src="/SopraMMS/js/jquery.multiple.select.js"></script>
